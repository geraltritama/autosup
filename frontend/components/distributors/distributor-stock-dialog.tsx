...
import { useDistributorStock } from "@/hooks/useDistributors";
...
export function DistributorStockDialog({ distributorId, distributorName, open, onClose }: Props) {
  const { data, isLoading } = useDistributorStock(open ? distributorId : null);

  return (
    <Dialog
      open={open}
      onClose={onClose}
      title={`Stok Distributor: ${distributorName}`}
      description="Visibilitas inventaris distributor partner secara real-time."
    >
      {isLoading ? (
